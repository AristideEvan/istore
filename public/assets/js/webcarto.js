...
function popUp(chemin,id){
    console.log("Chargement depuis :", chemin);
...
jQuery(document).ready(function() {
	...
	jQuery('#a-'+indice[tailleTab-2]).removeClass('collapsed');
	jQuery('#sd'+indice[tailleTab-2]).addClass('show');
  ...
  function Collapser(id){
	...
	jQuery('[id^="colapse-"]').each(function(){
		if($(this).prop('id')!='colapse-'+ordre){
			$(this).first().children('div').removeClass('show');
			$(this).first().children('a').removeClass('collapsed');
			//$(this).first().children('a').css('display','none');
