                                ...
                                <div class="col-md-8 offset-md-2">
                                    ...
                                    <fieldset>
                                        ...
                                        @foreach ($menus as $pkey=>$value)
                                            ...
                                            class="parent" id="m{{ $value[0]->menu_id }}"><label for="m{{ $value[0]->menu_id }}">{{ $value[0]->nomMenu }}</label><br>
                                            ...
                                            @foreach ($value[1] as $skey=>$sousMenu)
                                                ...
                                                @if(array_key_exists(1,$sousMenu))
                                                 @foreach ( $sousMenu[1] as $action )
                                                    <input type="checkbox" name="{{ $sousMenu[0]->menu_id }}[]" value="{{ $action->action_id }}" class="ssm fils{{ $sousMenu[0]->menu_id }} pfils{{ $value[0]->menu_id }}" id="a{{ $action->action_id }}"><label for="a{{ $action->action_id }}">{{ $action->nomAction }}</label><br>
                                                @endforeach
                                                @endif
                                            @endforeach
                                            @endif
