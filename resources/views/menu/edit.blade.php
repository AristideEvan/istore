        ...
        <div class="col-md-12">
            <div class="card">
                <div class="card-header py-0">{{ __('Ajouter menu') }}</div>
                    <div class="card-body">
                        <form class="needs-validation" novalidate method="POST" action="{{ route('menu.update',$menuConcerne->menu_id) }}">
                            @csrf
                            @method('PUT')
                            <div class="row">
                                <div class="col-md-6">
                                                ...
                                                <select id="parent" class="form-control @error('parent') is-invalid @enderror" name="parent"
                                                 autocomplete="parent" autofocus>
                                                    ...
                                                    @foreach ($parents as $parent)
                                                        <option value="{{ $parent->menu_id }}" @if($menuConcerne->parent_id==$parent->menu_id) selected @endif>{{ $parent->nomMenu }}</option>
                                                    @endforeach
                                                </select>
                                                    ...
                                                    <option value="{{ $action->action_id }}"
                                                        @if(count($tabAction)!=0)
                                                                @foreach ($tabAction as $item)
                                                                    @if($action->action_id==$item->action_id) selected @endif
                                                                @endforeach
